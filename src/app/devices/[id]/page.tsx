...
export default async function DeviceDetailPage({
  ...
  try {
    const [devRes, sessRes, statRes, dailyRes] = await Promise.all([
      ...
      pool.query(
        `SELECT s.id, s.cwd, s.started_at, s.last_turn_at, s.total_turns, s.total_cost_usd,
  ...
  return (
    <>
      ...
      <main className="page">
        ...
        {/* 헤더 */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 48, marginBottom: 48, alignItems: "end" }}>
          <div>
          ...
          <div style={{ textAlign: "right" }}>
            <div className="mono" style={{ fontSize: 11, color: "var(--ink-4)" }}>
              <div>첫 접속 {new Date(device!.first_seen).toLocaleDateString("ko-KR")}</div>
            </div>
          </div>
        </div>
            ...
            <tbody>
              {sessions.map((s) => (
                <ClickableTr key={s.id} href={`/sessions/${s.id}`} className="clickable">
                  ...
                  <td className="mono" style={{ fontSize: 12, color: "var(--ink-3)", maxWidth: "16rem" }}>
